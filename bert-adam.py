...
def get_target_data(filename):
    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            #Ignor convID, just grab target label, convert to tensor
...
class ClassifierNetwork(pl.LightningModule):
    def __init__(self, bert_model, max_conv_length):
...
def main():
    ...
    #Embed the data
    train_dataset = encoderNetwork(trainInput, trainLabels, tokenizer_bert)
    test_dataset = encoderNetwork(testInput,  testLabels, tokenizer_bert)

    #Make the dataloaders loggers and trainers
    train_dataloader = DataLoader(train_dataset, batch_size=TRAIN_BATCH_SIZE, shuffle=True, collate_fn=custom_collate_fn, num_workers=5)
    test_dataloader = DataLoader(test_dataset, batch_size=TEST_BATCH_SIZE, shuffle=False, collate_fn=custom_collate_fn, num_workers=5)

    logger = pl.loggers.CSVLogger("lightning_logs", name="ClassifierTest", version="bert-1")

    #Make classifier network
    classifier_network = ClassifierNetwork(model_bert, max_conv_length)
